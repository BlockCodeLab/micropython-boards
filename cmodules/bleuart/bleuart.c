...
static mp_obj_t build_advertising_payload(const char* name, size_t name_len) {
  ...
  size_t max_name_len = ADV_MAX_PAYLOAD - 5;  // 31 - 5 = 26
  ...
  size_t current_len = sizeof(flags);
  if (name_adv) current_len += name_adv_len;
  const size_t uuid_adv_len = 18;
  bool include_uuid = (current_len + uuid_adv_len <= ADV_MAX_PAYLOAD);
  size_t total_len = current_len;
  if (include_uuid) total_len += uuid_adv_len;
  ...
  if (include_uuid) {
    ...
    memcpy(payload + offset, uuid_adv, uuid_adv_len);
  }
...
static mp_obj_t bleuart_read(size_t n_args, const mp_obj_t* args) {
  ...
  if (n_args > 1) {
    sz = MIN(sz, mp_obj_get_int(args[1]));
  }
  if (sz == 0) {
    MICROPY_PY_BLUETOOTH_EXIT
