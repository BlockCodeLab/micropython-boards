...
static bool camera_init_helper(camera_obj_t *camera, size_t n_pos_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    if ((format != PIXFORMAT_JPEG) && (format != PIXFORMAT_YUV422) && (format != PIXFORMAT_GRAYSCALE) &&
        (format != PIXFORMAT_RGB565)) {
        mp_raise_ValueError(MP_ERROR_TEXT("Image format is not valid"));
    }

    int8_t size = args[ARG_framesize].u_int;
    if ((size != FRAMESIZE_96X96) && (size != FRAMESIZE_QQVGA) && (size != FRAMESIZE_QCIF) &&
        (size != FRAMESIZE_HQVGA) && (size != FRAMESIZE_240X240) && (size != FRAMESIZE_QVGA) &&
        (size != FRAMESIZE_CIF) && (size != FRAMESIZE_HVGA) && (size != FRAMESIZE_VGA) && (size != FRAMESIZE_SVGA) &&
        (size != FRAMESIZE_XGA) && (size != FRAMESIZE_HD) && (size != FRAMESIZE_SXGA) && (size != FRAMESIZE_UXGA) &&
        (size != FRAMESIZE_FHD) && (size != FRAMESIZE_P_HD) && (size != FRAMESIZE_P_3MP) && (size != FRAMESIZE_QXGA) &&
        (size != FRAMESIZE_QHD) && (size != FRAMESIZE_WQXGA) && (size != FRAMESIZE_P_FHD) &&
    ...
    camera->config.fb_count =
        args[ARG_FBSIZE].u_int; // if more than one, i2s runs in continuous mode. Use only with JPEG
