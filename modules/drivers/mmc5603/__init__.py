...
class MMC5603:
    ...
    def get_data(self):
        ...
        return (
            -0.0625 * (self.raw_x - self._offset_x),
            -0.0625 * (self.raw_y - self._offset_y),
            -0.0625 * (self.raw_z - self._offset_z),
        )
    ...
    def calibrate(self):
        ...
        print(
            "Save x_offset:{}, y_offset:{}, z_offset:{}".format(
    ...
    def get_strength(self):
        _x, _y, _z = self.get_data()
        return math.sqrt(math.pow(_x, 2) + pow(_y, 2) + pow(_z, 2))

    def get_angle(self, upright=False):
        _x, _y, _z = self.get_data()
        if upright:  # vertical
            angle = math.atan2(_z, -_x) * (180 / math.pi) + 180
            return angle if angle < 360 else angle - 360
        else:  # horizontal
            angle = math.atan2(_y, -_x) * (180 / math.pi) + 180
            return angle if angle < 360 else angle - 360
